...
If the [`F_UNSAFEMATH`](./index.md#flags) flag is set, an operation that would have panicked will instead set `$err` to `true`.

If the [`F_WRAPPING`](./index.md#flags) flag is set, an operation that would have panicked will instead set `$of` to the overflow of the operation.
